...
class SummaryPageSelonCapital extends StatefulWidget {
  final Map<String, dynamic> summaryData;
  final List<Map<String, dynamic>>? allData;
  final Map<String, dynamic> formData;

  SummaryPageSelonCapital({
  ...
  @override
  _SummaryPageSelonCapitalState createState() => _SummaryPageSelonCapitalState();
}

class _SummaryPageSelonCapitalState extends State<SummaryPageSelonCapital> {
  ...
  Widget build(BuildContext context) {
    // Calculer le gain fiscal total basé sur allData
    double gainFiscalTotal = widget.allData != null
        ? widget.allData!.fold(0.0, (sum, item) => sum + (item['mnt_gain_fiscal'] ?? 0.0))
        : widget.summaryData['mnt_gain_fiscal'] ?? 0.0;
          ...
          children: [
            Expanded(
              child: GridView.count(
                ...
                children: [
                  _buildCard('Épargne Constituée', widget.summaryData['mnt_epargne_constitu'].toString(),false),
                  _buildCard('Prime par ${_getReadableFractLabel(widget.summaryData['fract'] ?? '')}', widget.summaryData['mntprime_Fract'].toString(),true),
                  _buildCard('Gain Fiscal', widget.summaryData['mnt_gain_fiscal'].toString(),false),
                  _buildCard('Gain Financier', widget.summaryData['mnt_gain_financier'].toString(),true),
                ],
              ),
            ),
            SizedBox(height: 20),
                  ...
                  ElevatedButton(
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => DetailsPage(allData: widget.allData ?? [widget.summaryData]),
                        ),
                      );
                    },
                    child: Text(
                  ...
                  onPressed: () {
                    if (_formKey.currentState!.validate()) {
                      if (_selectedTypeRente == 'T' &&
                          _dureeRenteParAnController.text.isEmpty) {
                        print('Veuillez entrer la durée pour une rente temporaire.');
                      } else {
                        Navigator.of(context).pop();
                        _submitFormRente();
                        print('Type de rente choisi: $_selectedTypeRente');
                        print('Durée de la rente: ${_dureeRenteParAnController.text}');
                      }
                    } else {
...
Widget _buildDropdownField(String label, TextEditingController controller) {
  ...
  void _submitFormRente() async {
    ...
    if (_formKey.currentState!.validate()) {
      try {
        final response = await _apiService.simulateRenteParObjectif(
